...
public class ProgresoController : MonoBehaviour
{
    ...
    public void CargarYMostrarProgreso()
    {
        ...
        void ProcesarAnalytics(string json)
        {
            ...
            if (txtPartidasJugadas != null)
                txtPartidasJugadas.text = Mathf.RoundToInt(data.awareness_score).ToString();
        ...
        StartCoroutine(
            APIManager.Instance.GetAIRisk(
                ...
                {
                    SetPracticeButtonUnavailableState();
                    Debug.LogWarning("Error IA: " + error);
                }
            )
        );
    }
    ...
    void SetBotonVisual(Button boton, bool estaDesbloqueado)
    {
        ...
        if (estaDesbloqueado)
        {
            ...
            if (capaOpacidad != null) capaOpacidad.gameObject.SetActive(false);
            if (iconoCandado != null) iconoCandado.gameObject.SetActive(false);
        ...
        {
            boton.GetComponent<Image>().color = Color.white;
            if (capaOpacidad != null) capaOpacidad.gameObject.SetActive(true);
            if (iconoCandado != null) iconoCandado.gameObject.SetActive(true);
    ...
    private void ShowSurveyDiagnosticResult()
    {
        ...
        {
            txtAreaVulnerable.text = GetSurveyAreaDisplayName(weakness);
        }
        ...
        {
            txtEscenarioSugerido.text = GetSurveySuggestedPractice(weakness);
        }
    ...
    {
        if (string.IsNullOrEmpty(weakness)) return "";
    ...
    {
        if (string.IsNullOrEmpty(risk)) return "NO DISPONIBLE";
    ...
    private string ResolveRiskSource(AIRiskResponse data)
    {
        ...
        if (data.sufficient_behavioral_data || data.behavioral_decisions >= minDecisions)
        {
    ...
    {
        return data.min_behavioral_decisions > 0 ? data.min_behavioral_decisions : 3;
    }
    ...
    private void OnRiskLoaded(AIRiskResponse data)
    {
        ...
        string recommendation = BuildRiskRecommendationText(data, riskSource);
        ...
        if (txtAreaVulnerable != null)
        {
            txtAreaVulnerable.text = area;
        }

        if (txtRecomendacion != null)
        {
            txtRecomendacion.text = recommendation;
        }

        recommendedScenario = data.recommended_scenario;
        recommendationReady = AIState.IsValidPracticeScenario(recommendedScenario);
        SetPracticeButtonInteractable(recommendationReady);

        if (!recommendationReady)
        {
            Debug.LogWarning("Practicar: escenario recomendado no disponible para MiPerfil");
        }
        ...
        Debug.Log("🤖 Riesgo: " + data.risk_level);
        Debug.Log("📚 Área vulnerable: " + data.recommended_training);
        Debug.Log("💡 Recomendación: " + data.message);
    }
    ...
    public void PracticarEscenarioIA()
    {
        ...
        {
            Debug.LogWarning("Practicar: recomendación todavía no disponible");
            return;
        }

        AIState.SurveyResultPending = false;

        if (!recommendationReady || !AIState.IsValidPracticeScenario(recommendedScenario))
        {
            Debug.LogWarning("Practicar: recomendación todavía no disponible");
            return;
        }

        Debug.Log("🚀 Escenario recomendado: " + recommendedScenario);
