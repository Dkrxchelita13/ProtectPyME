...
CONCEPT_CATALOG = {
    ...
    "passwords.credential_request": _concept(
        "passwords.credential_request",
        "passwords",
        "Solicitud de credenciales",
        ["CREDENCIAL", "Credencial", "SOLICITUDCREDENCIALES", "Solicitud de credenciales"],
        "medio",
        "Peticion inesperada de una contrasena, codigo temporal o dato de acceso.",
        "Una solicitud legitima de soporte no debe pedir secretos completos por telefono, chat o correo. Si alguien pide credenciales, conviene detenerse, verificar por un canal oficial y reportar la situacion.",
        "Una llamada dice ser de soporte y pide tu contrasena para resolver un incidente urgente.",
        "Entregar una clave o codigo porque la persona parece pertenecer a la empresa.",
        "Solicitud sospechosa de contrasena o codigo.",
    ),
    "passwords.identity_verification": _concept(
        "passwords.identity_verification",
        "passwords",
        "Verificar identidad",
        ["VERIFICAR", "Verificar", "IDENTIDAD", "Identidad", "CANALOFICIAL", "Canal oficial"],
        "medio",
        "Confirmar por un canal oficial e independiente quien solicita informacion sensible.",
        "La verificacion evita confiar solo en una llamada, mensaje o nombre mostrado. Para proteger cuentas, se debe contactar al area responsable por un medio conocido antes de compartir cualquier informacion.",
        "Cuelgas una llamada dudosa y contactas a TI por el canal interno registrado.",
        "Usar el mismo numero o enlace que envio la persona sospechosa para confirmar la solicitud.",
        "Confirmar por canal oficial antes de responder.",
    ),
    ...
    "wifi.suspicious_traffic": _concept(
        "wifi.suspicious_traffic",
        "wifi",
        "Trafico sospechoso",
        ["TRAFICO", "Trafico", "TRAFICOSOSPECHOSO", "Trafico sospechoso", "ALERTA", "Alerta", "BLOQUEAR", "Bloquear"],
        "medio",
        "Flujo de informacion de red que no coincide con la actividad esperada.",
        "El trafico saliente inusual puede indicar que un equipo esta enviando datos sin autorizacion. La respuesta segura es revisar, bloquear o aislar segun el procedimiento y avisar a TI.",
        "Un equipo envia muchos datos a una direccion desconocida fuera del horario laboral.",
        "Ignorar la alerta porque el equipo todavia parece funcionar con normalidad.",
        "Actividad de red anomala que debe revisarse.",
    ),
    "wifi.data_exfiltration": _concept(
        "wifi.data_exfiltration",
        "wifi",
        "Exfiltracion de datos",
        ["EXFILTRACION", "Exfiltracion", "FUGADATOS", "Fuga datos", "FUGA", "Fuga", "DATOS", "Datos"],
        "medio",
        "Salida no autorizada de informacion desde un sistema o red.",
        "Una fuga de datos puede exponer clientes, pagos, documentos internos o credenciales. Detectarla temprano requiere observar transferencias inusuales y actuar antes de que la informacion salga de control.",
        "Un sistema intenta subir archivos internos a un destino externo no reconocido.",
        "Permitir la transferencia porque nadie reporto todavia un problema visible.",
        "Salida no autorizada de informacion.",
    ),
}
